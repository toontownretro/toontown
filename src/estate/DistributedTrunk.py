...
class DistributedTrunk(DistributedCloset.DistributedCloset):
    ...
    def setState(self, mode,
                 avId, ownerId,
                 gender,
                 hatList, glassesList, backpackList, shoesList):
    ...
    def load(self):
        # reparent the lid to the rotate node
        ...
        # determine scale of trunk so we can make the appropriate size trigger sphere
    ...
    def _handleCancel(self):
        if self.oldStyle:
            ...
            self.d_setDNA(oldHat[0], oldHat[1], oldHat[2],
                          ...
                          oldShoes[0], oldShoes[1], oldShoes[2],
                          1)
        else:
    ...
    def __handleSwapHat(self):
        item = self.av.getHat()
        self.d_setDNA(item[0], item[1], item[2],
    ...
    def __handleSwapBackpack(self):
        ...
        self.d_setDNA(N_A, N_A, N_A, N_A, N_A, N_A,
                      item[0], item[1], item[2],
                      N_A, N_A, N_A,
                      0, ToonDNA.BACKPACK)
        if self.closetGUI:
    ...
    def __handleSwapShoes(self):
        ...
        self.d_setDNA(N_A, N_A, N_A, N_A, N_A, N_A, N_A, N_A, N_A,
                      item[0], item[1], item[2], 0, ToonDNA.SHOES)
        if self.closetGUI:
            self.closetGUI.updateTrashButtons()
    ...
    def __handleDelete(self, which):
        ...
        if len(itemList) > 1:
            ...
            self.sendUpdate("removeItem", [trashItem[0], trashItem[1], trashItem[2], which])
    ...
    def _handlePurchaseDone(self, timeout = 0):
        ...
        if (timeout == 1):
            ...
            which = 0
            if hasattr(self.closetGUI, 'hatChoice') and hasattr(self.closetGUI, 'glassesChoice') and \
               hasattr(self.closetGUI, 'backpackChoice') and hasattr(self.closetGUI, 'shoesChoice'):
    ...
    def d_setDNA(self,
                 hatIdx, hatTexture, hatColor, glassesIdx, glassesTexture, glassesColor, 
    ...
    def setCustomerDNA(self, avId, hatIdx, hatTexture, hatColor,
                       glassesIdx, glassesTexture, glassesColor,
                       backpackIdx, backpackTexture, backpackColor,
    ...
    def __popupNotOwnerPanel(self):
        ...
        self.accept(self.cancelEvent, self._handleCancel)
        # special buttons if we own the trunk
        self.deleteEvent = self.uniqueName('delete')
        if (self.isOwner):
            self.accept(self.deleteEvent, self.__handleDelete)
