...
class ToonStyleManager:
    ...
    def loadNeighborhood(self, key, code):
        # Create the style samples
        self.createBaselineStyleAttributes(key, code)
        self.createWallStyleAttributes(key, code)
        self.createBuildingStyleAttributes(key, code)
        self.createColorAttributes(key)
        self.createDNAAttributes()
        self.createMiscAttributes()

    # BASELINE STYLE FUNCTIONS
    def createBaselineStyleAttributes(self, key, code):
        """
        ...
        attribute.setDict(
            ...
            self.createBaselineStyleDictionary(key, code))
        # Using this dictionary, create style pie menus
        attribute.setMenu(
            self.createBaselineStyleMenu(key, attribute.getDict()))
        dict[key] = attribute

    def createBaselineStyleDictionary(self, neighborhood, code):
        """
        ...
        return self.initializeBaselineStyleDictionary(styleData, neighborhood, code)

    def initializeBaselineStyleDictionary(self, styleData, neighborhood, code):
        """
    ...
    def createWallStyleAttributes(self, key, code):
        """
        ...
        attribute.setDict(
            ...
            self.createWallStyleDictionary(key, code))
        # Using this dictionary, create color pie menus
        attribute.setMenu(
            self.createWallStyleMenu(key, attribute.getDict()))
        dict[key] = attribute

    def createWallStyleDictionary(self, key, code):
        """
        Create a dictionary of wall styles for a neighborhood
        """
        filename = key + '_wall_styles.txt'
        print('Loading wall styles from: ' + filename)
        styleData = self.getStyleFileData(filename)
        return self.initializeWallStyleDictionary(styleData, code)

    def initializeWallStyleDictionary(self, styleData, code):
        """
    ...
    def createBuildingStyleAttributes(self, key, code):
        """
        ...
        for i in typeDict[key]:
            # Styles
            typeKey = i + '_styles'
            self.attributeDictionary[typeKey][key] = (
                typeAttributes[i])
        for i in NUM_WALLS:
            # Styles
            numWallKey = repr(i) + '_wall_styles'
            self.attributeDictionary[numWallKey][key] = (
                numWallsAttributes[i])
        for i in BUILDING_HEIGHTS:
            # Heights
            heightKey = repr(i) + '_ft_wall_heights'
            self.attributeDictionary[heightKey][key] = (
                heightAttributes[i])

    def createBuildingStyleDictionary(self, key, code):
        """
        Create a dictionary of wall styles for a neighborhood
        """
        filename = key + '_building_styles.txt'
        print('Loading building styles from: ' + filename)
        styleData = self.getStyleFileData(filename)
        return self.initializeBuildingStyleDictionary(styleData, code)

    def initializeBuildingStyleDictionary(self, styleData, code):
        """
    ...
    def createColorAttributes(self, key):
        # First compile color information for each neighborhood
        colorDictionary = {}
        colorMenuDictionary = {}
        colorDictionary[key] = (
            # Create a wall style dictionary for each neighborhood
            self.createColorDictionary(key))
        # Using this dictionary, create color pie menus
        colorMenuDictionary[key] = (
            self.createColorMenus(
            key, colorDictionary[key]))
        ...
        for colorType in COLOR_TYPES:
            ...
            colorList = colorDictionary[key][colorType]
            for i in range(len(colorList)):
                dict[i] = colorList[i]
            attribute.setDict(dict)
            attribute.setMenu(
                colorMenuDictionary[key][colorType])
            neighborhoodDict[key] = attribute
    ...
    def getColorDictionary(self, colorData):
        ...
        for line in colorData:
            ...
            if key in dict:
                ...
                print('ToonStyleManager.getColorDictionary key not found')
        return dict
