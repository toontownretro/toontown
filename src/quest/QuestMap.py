...
class QuestMap(DirectFrame):
        ...
        for questIndex in self.av.questPage.quests.keys():
            ...
            if not fComplete:
                if quest.getType() == Quests.RecoverItemQuest:
                    ...
                    continue
                elif quest.getType() != Quests.DeliverGagQuest and \
                     quest.getType() != Quests.DeliverItemQuest and \
                     quest.getType() != Quests.VisitQuest and \
                     quest.getType() != Quests.TrackChoiceQuest:
                    continue
    ...
    def initMarker(self, task):
        if self.av:
            if not hasattr(base.cr.playGame.getPlace(), 'isInterior') or \
               not base.cr.playGame.getPlace().isInterior:
                (relX, relY) = self.transformAvPos(self.av.getPos())
                self.marker.setPos(relX, 0, relY)
                self.marker.setHpr(0, 0, -180 - self.av.getH())
    ...
    def handleMarker(self):
        if hasattr(base.cr.playGame.getPlace(), 'isInterior') and \
