...
#    NUM_SUITS_SUITICUS_XTREMICUS = 50
    ...
    def __init__(self, air, zoneId):
        ...
        if simbase.air.wantCogdominiums:
            self.targetNumCogdos = \
              int(0.5 + self.SuitHoodInfo[self.hoodInfoIdx][self.SUIT_HOOD_INFO_BMIN] * self.CogdoRatio)
    ...
    def removeSuit(self, suit):
        ...
        assert self.numAttemptingCogdoTakeover == self.countCogdoTakeovers()
    ...
    def recycleBuilding(self, isCogdo):
        ...
        if target > bmin and current <= target:
            ...
            if isCogdo:
                ...
                if self.targetNumSuitBuildings > choice(self.MinimumOfOne, 1, 0):
    ...
    def assignInitialSuitBuildings(self):
        ...
        if simbase.air.wantCogdominiums:
            wantedCogdos = \
                int(wantedSuitBuildings * self.CogdoRatio)
        ...
        if simbase.air.wantCogdominiums:
            if actualCogdos > 0:
                # If we already have *some* cogdos in the world,
                # make sure they're all accounted for before we start
                # handing out more.
                numReassigned = 0
        ...
        if simbase.air.wantCogdominiums:
            if wantedCogdos > targetCogdos:
                ...
                # Hmm, we have to remove some targeted cogdos somewhere.
