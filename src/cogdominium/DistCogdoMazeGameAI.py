...
class DistCogdoMazeGameAI(DistCogdoGameAI, DistCogdoMazeGameBase):
    ...
    def suitHit(self, suitType, suitNum):
        ...
        if not toon:
            simbase.air.writeServerEvent('suspicious', avId,
                'CogdoMazeGame.suitHit: toon not present?')
            return False
    ...
    def requestHitBySuit(self, suitType, suitNum, networkTime):
        ...
        if self.fsm.getCurrentState().getName() != 'Game':
            self.logSuspiciousEvent(senderId,
                'CogdoMazeGameAI.requestHitBySuit outside of Game state')
            return False
    ...
    def requestHitByDrop(self):
        ...
        if self.fsm.getCurrentState().getName() != 'Game':
            self.logSuspiciousEvent(senderId,
                'CogdoMazeGameAI.requestHitByDrop outside of Game state')
            return False
    ...
    def requestGag(self, waterCoolerIndex):
        ...
        if self.fsm.getCurrentState().getName() != 'Game':
            self.logSuspiciousEvent(senderId,
            'CogdoMazeGameAI.requestGag outside of Game state')
            return False

        if waterCoolerIndex >= len(self._waterCoolerPosList):
            self.logSuspiciousEvent(senderId,
            'CogdoMazeGameAI.requestGag: invalid waterCoolerIndex')
            return
        ...
        if not toon:
            self.logSuspiciousEvent(senderId,
            'CogdoMazeGameAI.requestGag: toon not present')
            return
    ...
    def enterFinish(self):
        ...
        self.air.writeServerEvent('CogdoMazeGame',
            self._interior.toons,
            'Memos: %s/%s Weighted Memos: %s Time: %s Weighted Time: %s Score: %s' % (self.numPickedUp, self.maxPickups, weightedPickup, time, weightedTime, score),
            )
