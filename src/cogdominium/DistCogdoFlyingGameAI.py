    ...
    def _isPlayingGame(self, senderId):
        if self._gameState:
            ...
            self._reportSuspiciousEvent(
                senderId,
                'Client %s has made a game action request from illegal state: %s.' % (senderId, self.fsm.getCurrentState()),
                )
    ...
    def requestAction(self, action, data):
        ...
        if action == Globals.AI.GameActions.LandOnWinPlatform:
            ...
            if data in Globals.Gameplay.FuelStates:
                ...
                self._reportSuspiciousEvent(
                    senderId,
                    "Client %s has requested a fuel state that doesn't exist:%s." % (senderId, data),
                    )
