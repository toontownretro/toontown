...
from toontown.toonbase.ToontownModules import TextNode, NodePath
...
class CogdoFlyingFuelGui(DirectFrame):
    ...
    def _initModel(self):
        ...
        self.bladeNumberLabel = DirectLabel(
            ...
            relief = None,
            pos = (Globals.Gui.FuelNumBladesPos2D[0], 0, Globals.Gui.FuelNumBladesPos2D[1]),
    ...
    def _initIntervals(self):
        self._healthIval = LerpFunctionInterval(
            self.healthBar.setSz,
            fromData=0.0, toData=1.0, duration=2.0,
            )

        self.baseSpinDuration = 2.0
        self._spinIval = LerpFunctionInterval(
            self.propellerMain.setR,
            fromData=0.0, toData=-360.0, duration=self.baseSpinDuration,
            )
