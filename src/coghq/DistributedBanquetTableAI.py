...
class DistributedBanquetTableAI(DistributedObjectAI.DistributedObjectAI, FSM.FSM,
    ...
    def requestControl(self):
        ...
        if avId in self.boss.involvedToons and self.avId == 0 and self._state == 'Free':
    ...
    def forceControl(self, avId):
        ...
        if tableId == self.doId:
            if self._state == 'Flat':
                ...
                self.notify.warning('invalid forceControl from state %s' % self._state)
        else:
    ...
    def requestFree(self, gotHitByBoss):
        ...
        if avId == self.avId:
            if self._state == 'Controlled':
                ...
                self.notify.debug('requestFree denied in state %s' % self._state)
