...
class Fisherman(Toon.Toon):
    ...
    def __init__(self, id, toNpcId=20001, fAutonomous=1):
        """
        :param int id: local ID of fisherman
        :param int toNpcId: NPC ID for this fisherman's DNA, default is Flippy
        :param bool | int fAutonomous: If True, the Toon is automatically controlled. False allows the user to control.
        """
        ...
        if self.fAutonomous:
            ...
            self.targetButton = DirectButton(
    ...
    def getPole(self):
        toonTrack = Sequence(
            ...
            Parallel(
                ...
                ActorInterval(self.pole, 'cast', playRate = 0.5, duration = 27. / 12.),
                LerpScaleInterval(self.pole, duration = 2.0, scale = 1.0, startScale = 0.01)
            ),
        )
        toonTrack.play()
    ...
    def putAwayPole(self):
        Sequence(
            Parallel(
                ActorInterval(self, 'cast', duration = 1.0, startTime = 1.0, endTime = 0.0),
                ActorInterval(self.pole, 'cast', duration = 1.0, startTime = 1.0, endTime = 0.0),
    ...
    def holdPole(self):
        ...
        # One node, instanced to each of the toon's three right hands, will hold the pole.
    ...
    def initFish(self):
        ...
        self.wiggleIval = Sequence(
            self.fish.hprInterval(0.5, hpr = Point3(10, 0, 0), startHpr = Point3(-10, 0, 0), blendType = 'easeInOut'),
            self.fish.hprInterval(0.5, hpr = Point3(-10, 0, 0), startHpr = Point3(10, 0, 0), blendType = 'easeInOut')
        )
        ...
        self.circleIval = self.fishPivot.hprInterval(20, Point3(360, 0, 0), startHpr = Point3(0))
        self.circleIval.loop()

        taskMgr.remove('turnTask')
        taskMgr.doMethodLater(3.0 + random.random() * 3.0, self.turnFish, 'turnTask')
    ...
    def turnFish(self, state=None, targetPos=None):
        ...
        if fRightTurn:
            self.circleIval = self.fishPivot.hprInterval(20, Point3(currH - 360, 0, 0), startHpr = Point3(currH, 0, 0))
        else:
            self.circleIval = self.fishPivot.hprInterval(20, Point3(currH + 360, 0, 0), startHpr = Point3(currH, 0, 0))
        self.circleIval.loop()
        taskMgr.doMethodLater(3.0 + random.random() * 3.0, self.turnFish, 'turnTask')
    ...
    def makeTargetInterval(self):
        ...
        self.targetInterval = Sequence(
            self.target.posInterval(dur, self.targetEndPos, startPos = self.targetPos, blendType = 'easeInOut'),
            self.target.posInterval(dur, self.targetPos, startPos = self.targetEndPos, blendType = 'easeInOut'),
            name = 'moveInterval'
        )
